    ...
    <form method="get" action="{{ route('employees.index') }}">
      <div class="filters-grid">
        ...
        <div class="filter-group">
          ...
          <select name="cred" class="filter-select">
            ...
            <option value="huella" {{ ($credFilter ?? '') === 'huella' ? 'selected' : '' }}><i class="fas fa-fingerprint"></i> Con Huella</option>
            <option value="rostro" {{ ($credFilter ?? '') === 'rostro' ? 'selected' : '' }}><i class="fas fa-user"></i> Con Rostro</option>
            <option value="tarjeta" {{ ($credFilter ?? '') === 'tarjeta' ? 'selected' : '' }}><i class="fas fa-id-card"></i> Con Tarjeta</option>
      ...
      @if(($q ?? '') || ($credFilter ?? '') || (($orderBy ?? 'id_desc') !== 'id_desc'))
        <div class="active-filters">
          ...
          @if($credFilter ?? '')
            <span class="filter-tag">
              @if($credFilter === 'huella') <i class="fas fa-fingerprint"></i> Con Huella
              @elseif($credFilter === 'rostro') <i class="fas fa-user"></i> Con Rostro
              @elseif($credFilter === 'tarjeta') <i class="fas fa-id-card"></i> Con Tarjeta
...
<script>
  ...
  @if(session('fpStart') && session('fpStatus') && session('fpMark') && session('fpEmpId'))
    {{-- Pasamos las rutas de sesión a data-attributes --}}
    ...
    <script>
      window.addEventListener('load', async () => {
        ...
        try {
          ...
          // 2) Polling de estado (si tu firmware expone /Status)
